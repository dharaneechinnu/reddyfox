...
from content.models import Enquiry, QuoteRequest, RateLock
from .models import TeamAlert, TeamAlertDelivery, TeamPushSubscriber
from .services import notify_new_enquiry, notify_new_rate_lock, send_team_alert
...
def _make_rate_lock(**kwargs):
    ...
    defaults.update(kwargs)
    return RateLock.objects.create(**defaults)
...
class NotifyNewRateLockTests(TestCase):
    def test_alert_leads_with_the_figures_the_dealer_needs(self):
        lock = _make_rate_lock()
        TeamAlert.objects.all().delete()  # drop the one the signal already raised

        notify_new_rate_lock(lock)

        alert = TeamAlert.objects.get(lead=lock)
        self.assertEqual(alert.priority, TeamAlert.Priority.URGENT)
        self.assertIn(lock.name, alert.title)
        self.assertIn('USD->INR', alert.body)
        self.assertIn('84.0000', alert.body)

    def test_long_customer_name_does_not_overflow_the_title_column(self):
        # Lead.name allows 120 chars; TeamAlert.title is also 120, so an
        # untruncated "Rate lock — {name}" would overflow — and since this
        # runs in a post_save signal, that would roll back the customer's
        # rate lock entirely.
        lock = _make_rate_lock(name='A' * 120)
        alert = TeamAlert.objects.filter(lead=lock).first()
        self.assertIsNotNone(alert)
        self.assertLessEqual(len(alert.title), 120)
        self.assertLessEqual(len(alert.body), 255)

    def test_string_amount_does_not_break_the_save(self):
        # A caller may assign amount as a plain string; the instance still
        # holds it (not a Decimal) when the signal fires.
        lock = _make_rate_lock(amount='500', quoted_rate='84.0000')
        self.assertEqual(TeamAlert.objects.filter(lead=lock).count(), 1)
        self.assertIn('500.00', TeamAlert.objects.get(lead=lock).body)

    def test_a_broken_alert_never_loses_the_customers_rate_lock(self):
        with patch.object(TeamAlert.objects, 'create', side_effect=RuntimeError('boom')):
            lock = _make_rate_lock()
        # The lead itself must survive even though the alert blew up.
        self.assertIsNotNone(lock.pk)
        self.assertTrue(RateLock.objects.filter(pk=lock.pk).exists())
        self.assertEqual(TeamAlert.objects.count(), 0)


class LeadSignalTests(TestCase):
    """Brand-new enquiries and rate locks must both raise a push alert
    automatically — with no extra wiring — but quote requests and later edits
    must not."""
    ...
    def test_new_rate_lock_creates_alert(self):
        self.assertEqual(TeamAlert.objects.count(), 0)
        lock = _make_rate_lock()
        self.assertEqual(TeamAlert.objects.count(), 1)
        self.assertEqual(TeamAlert.objects.get().lead_id, lock.pk)
    ...
    def test_editing_an_existing_rate_lock_does_not_notify_again(self):
        lock = _make_rate_lock()
        self.assertEqual(TeamAlert.objects.count(), 1)

        lock.status = RateLock.Status.CONTACTED
        lock.save()
...
# This page extends the admin's own templates, which reference hashed static
# files. Production serves those from a manifest built by `collectstatic`;
# the test runner has no manifest, so fall back to plain storage rather than
# requiring a build step before `manage.py test`.
