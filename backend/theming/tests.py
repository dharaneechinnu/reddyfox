...
class TypeScaleTests(TestCase):
    ...
    def test_defaults_reproduce_the_original_hardcoded_sizes(self):
        # The refactor that introduced this app replaced ~180 inline sizes. At the default base
        # it must land on exactly the pixels those had, or that migration was not lossless.
    ...
    def test_heading_scale_lifts_headings_without_touching_body_text(self):
        ...
        variables = theme.css_variables
        self.assertEqual(variables['--fx-text-base'], '14.5px')  # body untouched
        self.assertEqual(variables['--fx-text-md'], '15.5px')
        self.assertEqual(variables['--fx-text-3xl'], '39px')     # 26 * 1.5
        self.assertEqual(variables['--fx-text-h1'], 'clamp(51px, 4vw, 84px)')
