...
- **`mcp_server`** — a [Model Context Protocol](https://modelcontextprotocol.io) endpoint at `POST /mcp/`, so staff can upload a photo or draft an FAQ by asking Claude or ChatGPT instead of learning the admin. **This is the only writer of content outside `/admin/` itself — `/api/` stays read-only, and `mcp_server/tests.py` asserts it.** Access is a bearer token (`McpToken`, stored hashed, scoped to any of read/images/content, revocable, optionally expiring), and every call is recorded in `McpCallLog`. The protocol is implemented directly in `protocol.py`/`views.py` rather than via the official SDK, which is ASGI — this project is WSGI/gunicorn, and a tools-only server is a few hundred lines of JSON-RPC. **The safety default that matters:** text created here (testimonials, FAQs) lands `is_visible=False` for a human to publish, because a fabricated quote or a wrong sentence about an RBI limit is a legal problem; photos land visible, because they make no claim and are replacing a placeholder. See `docs/mcp-server.md`.
...
- **Read-only API** — every public endpoint is a `ReadOnlyModelViewSet` (or a deliberately create-only view for leads). The website can never write to the database outside of submitting a lead; content changes only happen through an authenticated admin session, or through the token-authenticated `/mcp/` endpoint (see `mcp_server` above) — which is deliberately *not* under `/api/`, so "everything under `/api/` is safe to expose" stays true without exceptions.
