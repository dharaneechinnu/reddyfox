...
- **`telegram_alerts`** — a second, faster new-lead alert to staff, alongside (never instead of) the email in `content/notifications.py`. `TelegramSubscriber` (name, `chat_id`, `is_active`) is an admin-approved whitelist — a brand-new chat_id gets nothing until an admin creates its row. Two onboarding paths, both documented in `docs/telegram-bot.md`: a QR code (`TelegramInvite` — admin creates an invite with just a label, gets back a QR code + deep link, staff scans it and their subscriber row is created automatically) as primary, and a manual `getUpdates` chat_id lookup as the fallback. `telegram_alerts/updates.py::handle_update()` is the single function that turns a `/start <token>` message into a claimed invite + active subscriber — shared, unmodified, by both the production webhook (`views.py`) and the local-dev long-poll command (`telegram_poll_dev`, tests the whole flow with zero tunnel/ngrok setup). `TELEGRAM_WEBHOOK_SECRET` is one secret checked two ways: it's both the webhook URL's path segment and the `X-Telegram-Bot-Api-Secret-Token` header value Telegram echoes back — `set_telegram_webhook` registers it (or `--delete` removes it, e.g. before switching to `telegram_poll_dev` locally). `notify_team_telegram(lead)` is called from `content.views.BaseLeadCreateView.perform_create()` right alongside `notify_team(lead)`, same never-raises discipline, and logs one `TelegramDelivery` row per subscriber per lead (mirrors `notifications.NotificationDelivery`). `TELEGRAM_BOT_TOKEN` blank skips the send entirely, not an error.
- **`fx_providers`** — generic INR-denominated FX rate fetchers, with no knowledge of currency boards, margins, or anything else specific to `reference_rates`: `fetch_exchangerateapi()`, `fetch_fawazahmed0()`, `fetch_frankfurter()`, a `Provider` `TextChoices` (reusable in any model's dropdown), and `fetch_with_fallback(codes, primary=...)`, which tries `primary` first and walks the rest of `DEFAULT_ORDER` for whatever's still missing. Pulled into its own app deliberately narrow in scope — just the fetch functions, not a plugin framework — so a future feature that needs an INR rate can call it without depending on `reference_rates`. No models; registered in `INSTALLED_APPS` for consistency/future use. `EXCHANGERATE_API_KEY` (blank = skip that provider, not an error) is the only required env var.
- **`reference_rates`** — third-party mid-market FX rates (`ReferenceRate`), fetched via `fx_providers.fetch_with_fallback()`. `ReferenceRateSettings` (a `content.SiteSetting`-style singleton) holds `primary_provider` — **staff-switchable in the admin**, defaults to `exchangerate-api` (keyed, SLA-backed, 161 currencies; free tier 1,500 req/month against our actual ~60/month) — plus `auto_update_enabled` (default off) and one global `buy_margin`/`sell_margin` pair applied identically to every currency. Whichever provider isn't chosen as primary still serves as automatic fallback, in `fx_providers.DEFAULT_ORDER`. `reference_rates.services.refresh_reference_rates()` is the single entry point — called by the scheduled `fetch_reference_rates` management command (a Render Cron Job in production, once/24h), by a "Fetch reference rates now" admin action on `rates.CurrencyAdmin`, and by this app's own admin screen at `/admin/reference_rates/referenceratesettings/` — so all three do the same thing. It always writes `ReferenceRate`, and surfaces a read-only "Market ref" column on `CurrencyAdmin` (a typo guard, red past `REFERENCE_RATE_DIVERGENCE_WARN_PCT`, never blocks a save). `ReferenceRateSettingsAdmin` fully overrides `changelist_view()` to be one combined screen — the provider picker and margin form, and, after "Save & fetch now," a table right below it showing each currency's fetched market rate next to the buy/sell rate calculated from it — entirely owned by this app; `rates.CurrencyAdmin` only holds a link button to it, none of the logic. Read `docs/currency-rate-apis.md` before touching either app — it also covers why an unmodified market rate must never be the published one.
