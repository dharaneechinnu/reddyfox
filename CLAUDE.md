...
- **`telegram_alerts`** — a second, faster new-lead alert to staff, alongside (never instead of) the email in `content/notifications.py`. `TelegramSubscriber` (name, `chat_id`, `is_active`) is an admin-approved whitelist — a brand-new chat_id gets nothing until an admin creates its row. Two onboarding paths, both documented in `docs/telegram-bot.md`: a QR code (`TelegramInvite` — admin creates an invite with just a label, gets back a QR code + deep link, staff scans it and their subscriber row is created automatically by the production webhook) as primary, and a manual `getUpdates` chat_id lookup as the fallback for local dev (webhooks need a public HTTPS URL). The webhook (`telegram_alerts/views.py`, registered via `manage.py set_telegram_webhook`) checks Telegram's `X-Telegram-Bot-Api-Secret-Token` header against `TELEGRAM_WEBHOOK_SECRET` on every call — that, not the URL's unguessable path segment, is the real proof a request came from Telegram. `notify_team_telegram(lead)` is called from `content.views.BaseLeadCreateView.perform_create()` right alongside `notify_team(lead)`, same never-raises discipline, and logs one `TelegramDelivery` row per subscriber per lead (mirrors `notifications.NotificationDelivery`). `TELEGRAM_BOT_TOKEN` blank skips the send entirely, not an error.
