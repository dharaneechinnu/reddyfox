...
import { c, fs, fonts, wrap, eyebrow, h2Style } from '../tokens';
...
function Btn({ children, onClick, variant = 'primary', style }) {
  const base = { padding: '16px 28px', borderRadius: 9, fontSize: fs.md, fontWeight: 600, cursor: 'pointer', transition: 'background .18s,transform .18s', display: 'inline-block' };
  const variants = {
    primary: { background: c.orange, color: c.surface },
    outline: { border: '1px solid rgba(255,255,255,.26)', color: c.surface },
  };
  return (
    <span
      onClick={onClick}
...
export default function Home() {
  ...
  return (
    <div>
        ...
        <div style={{ position: 'relative', ...wrap, padding: '88px 32px 96px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(380px,100%),1fr))', gap: 64, alignItems: 'center' }}>
          <div style={{ animation: 'fx-up .7s ease both' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 9, border: '1px solid rgba(255,255,255,.18)', borderRadius: 100, padding: '7px 15px', marginBottom: 26 }}>
              ...
              <span style={{ fontFamily: fonts.mono, fontWeight: 500, fontSize: fs.xs, lineHeight: 1.4, letterSpacing: '.14em', color: c.onNavyText2, whiteSpace: 'nowrap' }}>AUTHORISED MONEY CHANGER</span>
            </div>
            <h1 style={{ fontFamily: fonts.serif, fontWeight: 400, fontSize: fs.hero, lineHeight: 1.04, letterSpacing: '-.015em', color: c.surface, margin: '0 0 22px' }}>One of the most reputed foreign currency exchanges in Chennai.</h1>
            <p style={{ fontSize: fs.xl, lineHeight: 1.62, color: c.onNavyText, maxWidth: 520, margin: '0 0 34px' }}>
            ...
            <div style={{ display: 'flex', gap: 34, flexWrap: 'wrap', borderTop: '1px solid rgba(255,255,255,.12)', paddingTop: 26 }}>
              {HERO_TRUST.map((t) => (
                <div key={t.label}>
                  <div style={{ fontFamily: fonts.mono, fontSize: fs['2xl'], color: c.surface, marginBottom: 5 }}>{t.value}</div>
                  <div style={{ fontSize: fs.sm, color: c.navyMuted2, letterSpacing: '.02em' }}>{t.label}</div>
                </div>
              ))}
            </div>
          </div>

          <div style={{ background: c.surface, borderRadius: 18, padding: 30, boxShadow: '0 40px 70px -30px rgba(0,0,0,.55)', animation: 'fx-up .7s .12s ease both' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
              <span style={{ fontSize: fs.xl, fontWeight: 600, color: c.navy }}>Currency converter</span>
              <span style={{ fontFamily: fonts.mono, fontWeight: 500, fontSize: fs['2xs'], lineHeight: 1.4, letterSpacing: '.12em', color: c.green, background: c.greenBg, padding: '6px 10px', borderRadius: 5, whiteSpace: 'nowrap' }}>TODAY'S COUNTER RATE</span>
            </div>

            <label style={{ display: 'block', fontSize: fs.sm, fontWeight: 500, color: c.textMuted, marginBottom: 8 }}>You Requirement</label>
            <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
              <input value={fx.amount} onChange={(e) => fx.setAmount(e.target.value)} inputMode="decimal" style={{ flex: 1, minWidth: 0, border: `1px solid ${c.softLine}`, borderRadius: 10, padding: '15px 16px', fontSize: fs['2xl'], fontFamily: fonts.mono, color: c.navy, outline: 'none' }} />
              <select value={fx.from} onChange={(e) => fx.setFrom(e.target.value)} style={{ border: `1px solid ${c.softLine}`, borderRadius: 10, padding: '15px 12px', fontSize: fs.md, fontWeight: 600, color: c.navy, background: c.sand, outline: 'none', cursor: 'pointer' }}>
      ...
      <section style={{ background: c.surface, borderBottom: `1px solid ${c.sandLine2}` }}>
        <div style={{ ...wrap, padding: '56px 32px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(200px,100%),1fr))', gap: 40 }}>
          {STATS.map((s) => (
            <div key={s.label}>
              <div style={{ fontFamily: fonts.serif, fontSize: fs.h2, lineHeight: 1, color: c.navy, marginBottom: 9 }}>{s.value}</div>
              <div style={{ fontSize: fs.base, fontWeight: 600, color: c.navyMid, marginBottom: 4 }}>{s.label}</div>
              <div style={{ fontSize: fs.sm, lineHeight: 1.5, color: c.textFaint }}>{s.note}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ background: c.sand, padding: '92px 0' }}>
        <div style={wrap}>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 32, flexWrap: 'wrap', marginBottom: 38 }}>
            ...
            <span onClick={() => navigate('/rates')} style={{ fontSize: fs.md, fontWeight: 600, color: c.navy, borderBottom: `1px solid ${c.orange}`, paddingBottom: 3, cursor: 'pointer' }}>View all 42 currencies</span>
          </div>
      ...
      <section style={{ background: c.surface, padding: '96px 0' }}>
        <div style={wrap}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 40, flexWrap: 'wrap', marginBottom: 44 }}>
            ...
            <p style={{ fontSize: fs.lg, lineHeight: 1.62, color: c.textMuted, maxWidth: 420, margin: 0 }}>From a ₹5,000 holiday buy to a ₹4 crore import settlement — handled by the same compliance team.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(320px,100%),1fr))', gap: 20 }}>
            {SERVICES.map((s) => (
              <div
                ...
                style={{ background: c.surface, border: `1px solid ${c.sandLine}`, borderRadius: 14, overflow: 'hidden', cursor: 'pointer', transition: 'box-shadow .22s,transform .22s,border-color .22s' }}
                onMouseEnter={(e) => { e.currentTarget.style.boxShadow = '0 24px 40px -28px rgba(11,27,51,.4)'; e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.borderColor = c.softLine; }}
              ...
              >
                ...
                <div style={{ padding: 30 }}>
                  ...
                  <p style={{ fontSize: fs.base, lineHeight: 1.62, color: c.textMuted, margin: '0 0 18px' }}>{s.short}</p>
      ...
      <section style={{ background: c.sand, padding: '96px 0' }}>
        <div style={{ ...wrap, display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(360px,100%),1fr))', gap: 64, alignItems: 'center' }}>
          ...
          <div>
            ...
            <h2 style={{ ...h2Style, fontSize: fs.h2, lineHeight: 1.1, marginBottom: 36 }}>The margin you see is the margin you pay</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 26 }}>
              {REASONS.map((f) => (
                <div key={f.n} style={{ display: 'flex', gap: 18 }}>
                  <span style={{ width: 30, height: 30, flex: 'none', borderRadius: 8, background: c.navy, color: c.surface, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: fonts.mono, fontWeight: 500, fontSize: fs.xs, lineHeight: 1.4 }}>{f.n}</span>
                  <div>
                    <h3 style={{ fontSize: fs.xl, fontWeight: 600, color: c.navy, margin: '3px 0 7px' }}>{f.title}</h3>
      ...
      {testimonials.length > 0 && (
        <section style={{ background: c.sand, padding: '92px 0' }}>
          <div style={wrap}>
            ...
            <h2 style={{ ...h2Style, fontSize: fs.h2, marginBottom: 40 }}>Trusted by travellers and finance teams</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(300px,100%),1fr))', gap: 20 }}>
              {testimonials.map((t) => (
                <div key={t.id} style={{ background: c.surface, border: `1px solid ${c.sandLine}`, borderRadius: 14, padding: 30, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  <p style={{ fontFamily: fonts.serif, fontSize: fs['2xl'], lineHeight: 1.45, color: c.ink, margin: '0 0 26px' }}>“{t.quote}”</p>
      ...
      {faqs.length > 0 && (
        <section style={{ background: c.sand, padding: '92px 0' }}>
          <div style={{ maxWidth: 1000, margin: '0 auto', padding: '0 32px' }}>
            ...
            <h2 style={{ ...h2Style, fontSize: fs.h2, marginBottom: 40, textAlign: 'center' }}>Before you walk in</h2>
      ...
      <section style={{ background: c.orange, padding: '80px 0' }}>
        <div style={{ ...wrap, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 40, flexWrap: 'wrap' }}>
          <div>
            <h2 style={{ fontFamily: fonts.serif, fontWeight: 400, fontSize: fs.h2, lineHeight: 1.1, color: c.surface, margin: '0 0 12px' }}>Call us to get quotes</h2>
            <p style={{ fontSize: fs.xl, lineHeight: 1.6, color: c.onOrangeText, margin: 0, maxWidth: 560 }}>
          ...
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'center' }}>
            {contact.mobiles.slice(0, 2).map((m) => (
              <a key={m.tel} href={`tel:${m.tel}`} style={{ background: c.navy, color: c.surface, padding: '16px 28px', borderRadius: 9, fontSize: fs.md, fontWeight: 600, fontFamily: fonts.mono, whiteSpace: 'nowrap' }}
