  ...
  return (
    <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(340px,100%),1fr))', gap: 56, alignItems: 'center' }}>
            <div>
              ...
              <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
                <span onClick={() => navigate(`/contact?service=${encodeURIComponent(active.title)}`)} style={{ background: c.orange, color: c.surface, padding: '15px 26px', borderRadius: 9, fontSize: fs.md, fontWeight: 600, cursor: 'pointer' }}>Request a quote</span>
      ...
      <section style={{ background: c.surface, padding: '88px 0' }}>
        <div style={wrap}>
          <p style={{ fontFamily: fonts.mono, fontWeight: 500, fontSize: fs.xs, lineHeight: 1.4, letterSpacing: '.18em', textTransform: 'uppercase', color: c.orange, margin: '0 0 14px' }}>Benefits</p>
          <h2 style={{ fontFamily: fonts.serif, fontWeight: 400, fontSize: fs.h2, lineHeight: 1.1, color: c.navy, margin: '0 0 40px' }}>What you get</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(280px,100%),1fr))', gap: 20 }}>
            {active.benefits.map(([title, body, logoSlot]) => (
              <div key={title} style={{ border: `1px solid ${c.sandLine}`, borderRadius: 14, overflow: 'hidden', padding: logoSlot ? 0 : 28 }}>
                {logoSlot && (
                  <SitePhoto
                    slot={logoSlot}
      ...
      <section style={{ background: c.sand, padding: '88px 0' }}>
        <div style={wrap}>
          <p style={{ fontFamily: fonts.mono, fontWeight: 500, fontSize: fs.xs, lineHeight: 1.4, letterSpacing: '.18em', textTransform: 'uppercase', color: c.orange, margin: '0 0 14px' }}>How it works</p>
          <h2 style={{ fontFamily: fonts.serif, fontWeight: 400, fontSize: fs.h2, lineHeight: 1.1, color: c.navy, margin: '0 0 44px' }}>Three steps, one visit</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(260px,100%),1fr))', gap: 24 }}>
            {active.process.map(([title, body], i) => (
              <div key={title} style={{ borderTop: `2px solid ${c.navy}`, paddingTop: 22 }}>
                <span style={{ fontFamily: fonts.mono, fontWeight: 500, fontSize: fs.xs, lineHeight: 1.4, letterSpacing: '.14em', color: c.orange }}>{'0' + (i + 1)}</span>
                <h3 style={{ fontSize: fs['2xl'], fontWeight: 600, color: c.navy, margin: '14px 0 9px' }}>{title}</h3>
                <p style={{ fontSize: fs.base, lineHeight: 1.62, color: c.textMuted, margin: 0 }}>{body}</p>
              </div>
            ))}
          </div>
      ...
      {faqs.length > 0 && (
        <section style={{ background: c.surface, padding: '88px 0' }}>
          <div style={{ maxWidth: 1000, margin: '0 auto', padding: '0 32px' }}>
            <h2 style={{ fontFamily: fonts.serif, fontWeight: 400, fontSize: fs.h2, lineHeight: 1.1, color: c.navy, margin: '0 0 36px', textAlign: 'center' }}>Common questions</h2>
